...
export default class Details extends MyMixin {
  ...
  async getRelatedQuestions() {
    ...
    let id = response.data.trivia_categories.find(
      (obj: any) => obj.name === this.quiz.category
    ).id;

    response = await instance.get(
      `https://opentdb.com/api.php?amount=4&type=multiple&category=${id}`
    );

    let data = response.data.results;
    data = data
      .filter((obj: any) => obj.question !== this.quiz.question)
      .splice(0, 3);
